...
export const prepareDeviceReducer = createReducerWithExtraDeps(initialState, (builder, extra) => {
    builder
        ...
        .addMatcher(
            action => action.type === METADATA.WALLET_LOADED || action.type === METADATA.WALLET_ADD,
            (state, { payload }: PayloadAction<{ deviceState: string; walletLabel?: string }>) => {
                updateMetadata(state, payload.deviceState, payload.walletLabel);
            },
        );
});
